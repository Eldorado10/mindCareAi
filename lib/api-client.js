...
// Admin Functions - Emergency Team
export async function fetchEmergencyTeamAdmin() {
  try {
    const response = await fetch(`${API_BASE}/admin/emergency-team`);
    if (!response.ok) throw new Error('Failed to fetch emergency team');
    return await response.json();
  } catch (error) {
    console.error('[API] Error fetching emergency team:', error);
    return null;
  }
}

export async function saveEmergencyTeamAdmin(data) {
  try {
    const response = await fetch(`${API_BASE}/admin/emergency-team`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || 'Failed to save emergency team');
    }
    return await response.json();
  } catch (error) {
    console.error('[API] Error saving emergency team:', error);
    throw error;
  }
}
